...
import { SaveOutlined, InfoCircleOutlined, EditOutlined, PhoneOutlined } from '@ant-design/icons';
...
interface EditCallModalProps {
  open: boolean;
  onCancel: () => void;
  onSubmit: (values: EditCallModalValues) => void;
  form: ReturnType<typeof Form.useForm<EditCallModalValues>>[0];
  loading: boolean;
  editingCallIndex: number | null;
}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: token.colorTextSecondary, lineHeight: '18px' }}>
              Звонок {editingCallIndex !== null ? editingCallIndex + 1 : ''}
            </div>
          </div>
        </div>
      }
    ...
    >
      <div style={{ padding: '0 24px' }}>
        ...
        >
          <Form.Item
            name="callName"
            label={<span style={{ fontWeight: 500, color: token.colorText, fontSize: '14px' }}>ID звонка</span>}
            rules={[
              { required: true, message: 'Пожалуйста, введите ID звонка' },
              { min: 3, message: 'ID звонка должен содержать минимум 3 символа' }
            ]}
            extra={<div style={{ fontSize: '12px', color: token.colorTextSecondary, marginTop: '4px' }}>Введите уникальный идентификатор звонка для отслеживания</div>}
            style={{ marginBottom: '20px' }}
          >
            <Input
              placeholder="Например: call_001"
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '11px', color: token.colorTextSecondary, lineHeight: '16px' }}>
                  ID звонка используется для связи с системой мониторинга. Убедитесь, что ID соответствует реальному идентификатору в вашей системе.
                </div>
              </div>
            </div>
          </div>
